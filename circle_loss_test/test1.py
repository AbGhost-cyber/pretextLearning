...
# siamese_test_model = SiameseModel2()
# state_dict = torch.load('/Users/mac/Downloads/circle_loss_0.5_margin_hindi1.pt', map_location=torch.device('cpu'))
# siamese_test_model.load_state_dict(state_dict)
# # evaluate2(siamese_test_model, criterion, test_loader)
#
# # Get the triplet embeddings and labels
# embeddings = []
# labels = []
#
# # Generate random anchor, positive, and negative embeddings
# with torch.no_grad():
#     for anchor, positive, negative in test_loader:
#         # Assuming your model returns a list of triplet embeddings
#         anchor, positive, negative = siamese_test_model(anchor, positive, negative)
#         embeddings.extend([anchor, positive, negative])
...
#
#     # Plot the embeddings with different colors based on their roles
#     colors = {"Anchor": "red", "Positive": "green", "Negative": "blue"}
#
#     for i, label in enumerate(labels):
#         plt.scatter(embeddings_tsne[i, 0], embeddings_tsne[i, 1], color=colors[label])
#
#     # Add a legend
#     legend_elements = [
#         plt.Line2D([0], [0], marker='o', color='w', label='Anchor', markerfacecolor='red', markersize=8),
#         plt.Line2D([0], [0], marker='o', color='w', label='Positive', markerfacecolor='green', markersize=8),
#         plt.Line2D([0], [0], marker='o', color='w', label='Negative', markerfacecolor='blue', markersize=8)
#     ]
#
...
hindi_folder = ImageFolder(root="data/BHSig260/Hindi")
test_hindiDataset = HindiSignatureDataset(imageFolder=hindi_folder, transform=image_transform, isTrain=False)
test_loader = DataLoader(test_hindiDataset, batch_size=40, shuffle=False)

siamese_test_model = SiameseModel2().to(device)
state_dict = torch.load('circle_loss_0.5_margin_hindi1.pt')
siamese_test_model.load_state_dict(state_dict)
# evaluate2(siamese_test_model, criterion, test_loader)
...
with torch.no_grad():
    ...
    # Convert embeddings to numpy array and reshape
    embeddings_np = embeddings.view(embeddings.size(0), -1).cpu().numpy()

    # Apply t-SNE to reduce dimensionality to 2
    tsne = TSNE(n_components=2, perplexity=128, random_state=42)
    embeddings_tsne = tsne.fit_transform(embeddings_np)
...
def calculate_far(model, dataloader, threshold):
    false_acceptances = 0
    ...
    with torch.no_grad():
        for anchor, positive, negative in dataloader:
            ...
            # Calculate distances/similarities using custom criterion
            positive_distance = torch.pairwise_distance(anchor_embedding, positive_embedding, p=2)
            negative_distance = torch.pairwise_distance(anchor_embedding, negative_embedding, p=2)

            # Check for false acceptances
            false_acceptances += torch.sum(negative_distance < threshold).item()
            total_negative_pairs += negative.size(0)

    far = false_acceptances / total_negative_pairs
    return far
